...
class Menu extends React.Component {
  ...
  handleChapterSelection = (event) => {
    ...
    if (this.state.selectedbookId === 0) // if using default selection (only changing chapter)
      this.props.history.push(`/${this.props.match.params.book}/${parseInt(event.target.value)}`);
    else
      this.props.history.push(`/${this.state.selectedbookId}/${parseInt(event.target.value)}`);
  ...
  renderPrevChLink = (book, chapter) => {    
    if (book === 1 && chapter === 1) {
      ...
      return (
        <Link to={`/${prevBook}/${prevCh}`} className={classNames({"disable-link": this.props.menuOpen})}>
          <div className="prev">
  ...
  renderNextChLink = (book, chapter) => {    
    ...
    if (book === 66 && chapter === 22) {
      ...
      return (
        <Link to={`/${nextBook}/${nextCh}`} className={classNames({"disable-link": this.props.menuOpen})}>
          <div className="next">
